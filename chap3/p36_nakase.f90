...
subroutine calc(a, bec, bec_2, bec2, bec2_2, ev)
  complex(8), intent(in) :: bec(1, 2), bec_2(1,2), ev(2)
  real(8), intent(in) :: bec2(1, 2), bec2_2(1, 2), a(2, 2)
  real(8) p(2, 2) , b, c, d
  complex(8) q(2, 2)
  integer i
  b  = - 0.5d0 * (a(1, 1) + a(2, 2))
  ...
  if ( d < 0 ) then
    ...
    write(*,*) matmul(q(1, 1:2), transpose(bec) )
    write(*,*) matmul(q(2, 1:2), transpose(bec_2))
  else
    p(:, :) = a(:, :)
    do i = 1, 2
      p(i, i) = a(i, i) - ev(i)
    ...
    write(*,*) matmul(p(1, 1:2), transpose(bec2) )
    write(*,*) matmul(p(2, 1:2), transpose(bec2_2))
  end if
end subroutine calc
end module
...
program ensyu36
 ...
 call calc(a, bec, bec_2, bec2, bec2_2, ev)
end program ensyu36
